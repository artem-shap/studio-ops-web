...
describe("the Zod schema matches the published contract", () => {
    ...
    it("agrees on the minimum length of message", () => {
        ...
        expect(inquirySchema.safeParse(valid({ message: "a".repeat(min!) })).success).toBe(true);
        expect(inquirySchema.safeParse(valid({ message: "a".repeat(min! - 1) })).success).toBe(false);
    });
